import { minutesOfDay, timeHM } from '../utils/time'

const TYPE_LABEL = {
  'Лекция': { label: 'Лекция', cls: 'badge--lecture' },
  'Практическое занятие': { label: 'Практика', cls: 'badge--practice' },
  'Лабораторная работа': { label: 'Лаб. работа', cls: 'badge--practice' },
}
...
function calcProgress(timeBegin, timeEnd) {
  const start = minutesOfDay(timeBegin)
  const end = minutesOfDay(timeEnd)
  ...
  return (
    <div className={cardClass}>
      ...
      <div className="lesson-index">
        <span className="lesson-index-num">{lesson.pair_number ?? '–'}</span>
        <span className="lesson-index-label">пара</span>
      </div>

      <div className="lesson-body">
        <div className="lesson-time">
          {timeHM(lesson.time_begin)}–{timeHM(lesson.time_end)}
        </div>
