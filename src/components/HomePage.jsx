...
export default function HomePage({ weeks }) {
  ...
  const lastEnd = minutesOfDay(lessons[lessons.length - 1].time_end)
  const firstStart = minutesOfDay(lessons[0].time_begin)
  const afterAll = cur > lastEnd
  const beforeAll = cur < firstStart

  let statusText = null
  if (nowIndex !== -1) {
    const remaining = minutesOfDay(lessons[nowIndex].time_end) - cur
    statusText = {
      type: 'active',
      text: `Сейчас идёт пара ${lessons[nowIndex].pair_number ?? nowIndex + 1}`,
      sub: `осталось ${minutesLabel(remaining)}`,
    }
  } else if (!afterAll && nextIndex !== -1) {
    const until = minutesOfDay(lessons[nextIndex].time_begin) - cur
    statusText = {
      type: 'next',
    ...
    <div className="home-page">
      ...
      <div className="lessons-list">
        {lessons.map((lesson, i) => {
          ...
          let breakMins = 0
          if (prev) {
            breakMins = minutesOfDay(lesson.time_begin) - minutesOfDay(prev.time_end)
          }
